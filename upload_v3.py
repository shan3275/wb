...
def upload():
    ...
    print yesterday_str
    real_file = '%s/rdup.log.%s' %(options.ipath, yesterday_str)
    ...
    #cmd = '/usr/bin/curl  -F "src=@./%s" http://114.67.29.253:8185/cookie.catchs.dev/api/upload' %(ency_file)
    cmd = '/usr/bin/curl  -F "src=@./%s" http://0.0.0.0:5000/log?plateform\=weibo\&type\=normal\&src\=hehe' %(ency_file)
